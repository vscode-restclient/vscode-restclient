...
async function enviar(
  ...
  const abiertos = vscode.workspace.textDocuments
    .map(d => `${d.languageId}:${d.getText().slice(0, 50)}`)
    .join(" | ")
  throw new Error(
...
describe("HttpKeeper · peticiones reales", () => {
  ...
  describe("lo básico", () => {
    ...
    it("P-08 · varias peticiones separadas por ###, se envía la del cursor", async () => {
      ...
      assert.ok(
        t.includes("/primera"),
        "debe enviarse la petición donde está el cursor"
      )
      ...
      for (let i = 0; i < 60 && !texto.includes("original"); i++) {
        ...
        texto =
          vscode.workspace.textDocuments
            ...
            .map(d => d.getText())
            .find(t => t.includes("/reenvio")) ?? ""
      }
      assert.ok(
        texto.includes("original"),
        `el reenvío perdió la cabecera:` + BR + texto.slice(0, 250)
      )
  ...
  describe("compatibilidad", () => {
    it("P-10 · un ajuste propio se aplica", async () => {
      ...
      try {
        const t = await enviar(
          `GET ${BASE}/cabeceras` + BR,
          "httpkeeper-propio"
        )
        assert.ok(
          t.includes("httpkeeper-propio"),
          "no se aplicó la cabecera por defecto"
        )
      } finally {
    ...
    it("P-16 · sin ajuste propio se hereda el de REST Client", async () => {
      ...
      assert.ok(
        t.includes("viene-de-restclient"),
        "no se heredó la configuración de REST Client"
      )
    })
...
describe("HttpKeeper · resolución de localhost", () => {
  ...
  it("P-27 · localhost alcanza un servidor que solo escucha en IPv6", async () => {
    const t = await enviar(
      `GET ${BASE_LOCALHOST}/por-nombre` + BR,
      "/por-nombre"
    )
    assert.ok(
      t.includes("HTTP/1.1 200"),
      `no llegó por localhost:
${t.slice(0, 200)}`
    )
  })
})
...
describe("HttpKeeper · variables de petición", () => {
    ...
    for (let i = 0; i < 60 && !lista; i++) {
      ...
      lista = vscode.workspace.textDocuments.some(
        d =>
          ...
          d.getText().includes("HTTP/1.1 200") &&
          d.getText().includes("x-ruta: " + ruta)
      )
    }
  ...
  it("P-12 · JSONPath extrae un valor de la respuesta anterior", async () => {
    ...
    assert.ok(
      t.includes("/eco-json?v=1"),
      `el JSONPath no se resolvió:\n${t.slice(0, 250)}`
    )
  })
  ...
  it("P-13 · XPath extrae un valor de una respuesta XML", async () => {
    ...
    assert.ok(
      t.includes("/eco-xml?v=valor"),
      `el XPath no se resolvió:\n${t.slice(0, 250)}`
    )
  })
  ...
  it("P-29 · un JSONPath con varios resultados los devuelve todos", async () => {
    const t = await encadenar(
      `# @name uno` +
        ...
        BR +
        BR +
        `GET ${BASE}/eco-lista?v={{uno.response.body.$.items[*].id}}` +
        BR,
      "/eco-lista"
    )
    assert.ok(
      t.includes("7") && t.includes("9"),
      `deberían venir los dos identificadores:` + BR + t.slice(0, 250)
    )
  })
